...
export function getReadmeRawUrl(
  repoUrl: string,
  branch?: string,
  readmeFile?: string,
) {
  if (
    /^https\:\/\/raw\.githubusercontent\.com\//i.test(repoUrl) &&
    /readme\.md$/.test(repoUrl)
  ) {
    return repoUrl;
  } else if (
    /^https\:\/\/raw\.githack\.com\//i.test(repoUrl) &&
    /readme\.md$/.test(repoUrl)
  ) {
    return repoUrl;
  }
...
export async function getRepositories(repoList: string[]) {
  ...
  if (data.errors) {
    throw Error(
      "Failed to query repositories with errors " + JSON.stringify(data.errors),
    );
  }
